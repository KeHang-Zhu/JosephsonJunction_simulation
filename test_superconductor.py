...
def make_system(a=1, W=10, L=10, barrier=0, barrierpos=(3, 4),
                mu=0.4, Delta=0.1, Deltapos=4, t=1.0, t2= 2.0):
    ...
    syst[(lat(x, y) for x in range(Deltapos) for y in range(W))] = (4 * t - mu) * tau_z + Delta * tau_x
    ...
    lead0[(lat(0, j) for j in range(W))] = (4 * t*2 - mu*2) * tau_z+ Delta * tau_x 
    lead0[lat.neighbors()] = -2*t * tau_z
...
def test_main():
    ...
    plot_conductance(syst, energies=[0.002 * i for i in range(-10, 400)])
