...
def make_system(a=1, W=10, L=10, barrier=1.5, t2 = 1.0,
    ...
    syst[(lat(x, y) for x in range(L) for y in range(W))] = (4 * t - mu) * tau_z + Delta * tau_x
    
    #### Define the Right SC region. ####
    syst[(lat(x, y) for x in range(L+Metal_width, 2*L+Metal_width) for y in range(W))] = (4 * t - mu) * tau_z + Delta * tau_x
    
    #### Define the Normal metal region. ####
    syst[(lat(x, y) for x in range(L, L+Metal_width)
    ...
    lead1[(lat(0, j) for j in range(W))] = (4 * t - mu) * tau_z 
